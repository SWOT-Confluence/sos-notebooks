...
```{r}
#plot USGS width vs SWOT width
##Download all ADCP data from a site

# download_ADCP <- function(SiteID = site_no, startDate = start, endDate = end){
#   raw.data = readNWISmeas(SiteID, tz = "UTC", startDate, endDate, expanded = TRUE)
#   
#   ##Read in Site Name
#   sitename <- readNWISsite(SiteID)
#   raw.data = raw.data[, c("measurement_dateTime", "site_no", "gage_height_va", "discharge_va",
#                           "chan_discharge", "chan_width", "chan_area", "chan_velocity", "measured_rating_diff", 
#                           "meas_type", "streamflow_method", "velocity_method")]
#   
#   #Add decimal date, depth, and remove NAs
#   raw.data <- raw.data %>% mutate(DD = decimal_date(measurement_dateTime)) %>% 
#     mutate(chan_depth = chan_area/chan_width) %>%
#     mutate(Fr = (chan_velocity/sqrt(32.17405*chan_depth))) %>%
#     drop_na()
#   return(raw.data)
# }
# 
# ptm = proc.time()
# USGS_SWOT_ADCP_measurements = do.call(rbind, lapply(SWORD_v16_USGS_gages$site_no, possibly(download_ADCP, otherwise = NULL)))
# proc.time() - ptm
# 

#plot (static) NHD slope vs SWOT width
